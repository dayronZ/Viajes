...
export default function CotizadorConMapa() {
  ...
  const [tipoViaje, setTipoViaje] = React.useState("ida_vuelta");
  const [pasajeros, setPasajeros] = React.useState(1);
  const [maletas, setMaletas] = React.useState(0);
  const [clase, setClase] = React.useState("comercial");
  const [mostrarDirectos, setMostrarDirectos] = React.useState(false);
  ...
  return (
    <div className="cotizador-container">
      ...
      <h2>Cotiza tu vuelo ida y vuelta</h2>
      ...
      <div>
        ...
        {!vuelosFiltrados.length && (
          <p className="mensaje-info">Elige origen, destino, fechas y demás para ver opciones.</p>
        )}
        <div className="vuelos-list">
          {vuelosFiltrados.map((v) => (
            <div key={v.id} className="vuelo-card" tabIndex={0}>
